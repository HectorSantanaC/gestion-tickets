...
{
  return $page === $current ? 'text-primary dark:text-inverse-primary font-bold border-r-4 border-primary bg-surface-container-high dark:bg-secondary-container' : 'text-on-surface-variant dark:text-surface-variant hover:bg-surface-container-high dark:hover:bg-secondary-container';
}
?>

<!-- SideNavBar -->
<aside class="flex flex-col h-screen fixed left-0 top-0 py-margin-xl bg-surface dark:bg-inverse-surface docked h-full w-64 border-r border-outline-variant dark:border-outline shadow-sm z-50">
  <div class="px-6 mb-8">
  ...
  <nav class="flex-1 space-y-1 px-3">
    <?php foreach ($navItems as $key => $item): ?>
      <a class="flex items-center px-3 py-3 font-body-md <?php echo isActive($key, $currentPage); ?> transition-all"
  ...
  <div class="mt-auto px-3 space-y-1">
    <?php foreach ($bottomItems as $key => $item): ?>
      <a class="flex items-center px-3 py-3 font-body-md text-on-surface-variant dark:text-surface-variant hover:bg-surface-container-high dark:hover:bg-secondary-container transition-all"
        href="<?php echo $item['href']; ?>">
        <span class="material-symbols-outlined mr-3" data-icon="<?php echo $item['icon']; ?>"><?php echo $item['icon']; ?></span>
        <span><?php echo $item['label']; ?></span>
      </a>
